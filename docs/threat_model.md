...
- **Limited real-world OOD validation.** Real night and glare have been tested and do not induce the collapse (`report/real_weather_results.md`); rain, fog, novel geography, sensor degradation, and adversarial perturbations on real footage remain project-pending. The candidate non-CARLA OOD axes and their trade-offs are enumerated in `docs/paper_plan.md` §3; generalizing E6 across simulator engines and real-world OOD stimuli is the stated next step.
